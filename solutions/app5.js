const sortByPrice = (data = []) => data.sort((a, b) => a.price - b.price);

const filterByPrice = (data = [], minPrice = 25) => data
...
export const getProducts = async (url = '') => {
  try {
    ...
    const sorted = sortByPrice(data);
    const filtered = filterByPrice(sorted, 25);
    return filtered;
  } catch (err) {
    console.error(err);
    return [];
